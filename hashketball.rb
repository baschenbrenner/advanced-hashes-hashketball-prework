require 'pry'
def game_hash
   {
  :away=>{
    :team_name=>"Charlotte Hornets",
    :colors=>["Turquoise","Purple"],
    :players=>{
      "Jeff Adrien" => {
        :number => 4,
        :shoe => 18,
        :points => 10,
        :rebounds => 1,
        :assists => 1,
        :steals => 2,
        :blocks => 7,
        :slam_dunks => 2
      },
      "Bismack Biyombo" => {
        :number => 0,
        :shoe => 16,
        :points => 12,
        :rebounds => 4,
        :assists => 7,
        :steals => 22,
        :blocks => 15,
        :slam_dunks => 10
      },
      "DeSagna Diop" => {
        :number => 2,
        :shoe => 14,
        :points => 24,
        :rebounds => 12,
        :assists => 12,
        :steals => 4,
        :blocks => 5,
        :slam_dunks => 5
      },
      "Ben Gordon" =>   {
          :number => 8,
          :shoe => 15,
          :points => 33,
          :rebounds => 3,
          :assists => 2,
          :steals => 1,
          :blocks => 1,
          :slam_dunks => 0
         },
      "Kemba Walker" => {
        :number => 33,
        :shoe => 15,
        :points => 6,
        :rebounds => 12,
        :assists => 12,
        :steals => 7,
        :blocks => 5,
        :slam_dunks => 12
      }

    }
  },
  :home=>{
    :team_name=>"Brooklyn Nets",
    :colors=>["Black","White"],
    :players=>{
      "Alan Anderson" => {
        :number => 0,
        :shoe => 16,
        :points => 22,
        :rebounds => 12,
        :assists => 12,
        :steals => 3,
        :blocks => 1,
        :slam_dunks => 1
      },
      "Reggie Evans" => {
        :number => 30,
        :shoe => 14,
        :points => 12,
        :rebounds => 12,
        :assists => 12,
        :steals => 12,
        :blocks => 12,
        :slam_dunks => 7
      },
      "Brook Lopez" => {
        :number => 11,
        :shoe => 17,
        :points => 17,
        :rebounds => 19,
        :assists => 10,
        :steals => 3,
        :blocks => 1,
        :slam_dunks => 15
      },
      "Mason Plumlee" =>   {
          :number => 1,
          :shoe => 19,
          :points => 26,
          :rebounds => 12,
          :assists => 6,
          :steals => 3,
          :blocks => 8,
          :slam_dunks => 5
        },
      "Jason Terry" => {
        :number => 31,
        :shoe => 15,
        :points => 19,
        :rebounds => 2,
        :assists => 2,
        :steals => 4,
        :blocks => 11,
        :slam_dunks => 1
      }}
  }
}
end

def num_points_scored(player_name)
  if game_hash[:away][:players][player_name]
    return game_hash[:away][:players][player_name][:points]
  else
    return game_hash[:home][:players][player_name][:points]
  end
end

def shoe_size(player_name)
  game_hash[:away][:players][player_name] ? game_hash[:away][:players][player_name][:shoe] : game_hash[:home][:players][player_name][:shoe]
end

def team_colors(team_name)
  game_hash[:away][:team_name] == team_name ? game_hash[:away][:colors] : game_hash[:home][:colors]
end

def team_names
  [game_hash[:away][:team_name],game_hash[:home][:team_name]]
end

def player_numbers(team_name)

  team_numbers = []
  if game_hash[:away][:team_name] == team_name
    game_hash[:away][:players].each do |player_name, stats|
      team_numbers << stats[:number]
    end
  else
    game_hash[:home][:players].each do |player_name, stats|
      team_numbers << stats[:number]
    end
  end
  team_numbers
end

def player_stats(player_name)
  if game_hash[:away][:players][player_name]
    return game_hash[:away][:players][player_name]
  else
    return game_hash[:home][:players][player_name]
  end
end

def big_shoe_rebounds
  biggest_shoe_size= 0
  corresponding_rebounds = 0
  game_hash.each do |location, team_info|
    team_info[:players].each do |player_name, stats|
      if stats[:shoe] > biggest_shoe_size
        biggest_shoe_size = stats[:shoe]
        corresponding_rebounds = stats[:rebounds]
      end
    end
  end
  return corresponding_rebounds
end

def most_points_scored

  most_points_scored = 0
  high_scorer = ""
  game_hash.each do |location, team_info|
    team_info[:players].each do |player_name, stats|
      if stats[:points] > most_points_scored
        most_points_scored = stats[:points]
        high_scorer = player_name
      end
    end
  end
  return high_scorer
end

def winning_team
  away_points = 0
  home_points = 0
  game_hash.each do |location, team_info|
    if team_info[:location] = :away
      team_info[:players].each do |player_name, stats|
        away_points += stats[:points]
      end
    end
    if team_info[:location] = :home
      team_info[:players].each do |player_name, stats|
        home_points += stats[:points]
      end
    end
  end
  away_points > home_points ? game_hash[:away][:team_name] : game_hash[:home][:team_name]
end

def player_with_longest_name
  length_of_longest_name = 0
  longest_name = ""
  game_hash.each do |location, team_info|
    team_info[:players].each do |player_name, stats|
      if player_name.length > length_of_longest_name
        length_of_longest_name = player_name.length
        longest_name = player_name
      end
    end
  end
  return longest_name
end

def most_steals

  high_number_of_steals = 0
  high_scorer = ""
  game_hash.each do |location, team_info|
    team_info[:players].each do |player_name, stats|
      if stats[:steals] > high_number_of_steals
        high_number_of_steals = stats[:steals]
        high_scorer = player_name
      end
    end
  end
  return high_scorer
end

def long_name_steals_a_ton?
  most_steals == player_with_longest_name
end
